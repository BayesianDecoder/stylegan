...
def eval_experiment(
    ...
    if distance_metrics is None:
        distance_metrics = [
            torchmetrics.PeakSignalNoiseRatio(data_range=2.0).to(_DEVICE),
            lpips.LearnedPerceptualImagePatchSimilarity(net_type="vgg").to(_DEVICE),
        ]
    for suffix in suffixes:
        if COMPUTE_FID := True:
            ...
            fid_score = float(result.decode("utf8").strip().splitlines()[-1].replace("FID:  ", ""))
            json.dump(
                fid_score,
                open(
            ...
            for im_path in globr(f"{expr_path}/inversions/**/pred{suffix}.png"):
                ...
                for metric in distance_metrics:
                    degraded_scores[accronym(metric)].append(
                        metric(degraded_pred.to(_DEVICE), target.to(_DEVICE)).item()
                    )
                    ground_truth_scores[accronym(metric)].append(
                        metric(pred.to(_DEVICE), ground_truth.to(_DEVICE)).item()
                    )
