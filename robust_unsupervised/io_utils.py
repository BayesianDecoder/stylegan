...
def open_generator(pkl_path: str, refresh=True, float=True, ema=True) -> networks.Generator:
    ...
    with dnnlib.util.open_url(pkl_path) as fp:
        G = legacy.load_network_pkl(fp)["G_ema" if ema else "G"].to(DEVICE).eval()
        if float:
            G = G.float()
    ...
    if refresh:
        with torch.no_grad():
            old_G = G
            G = networks.Generator(*old_G.init_args, **old_G.init_kwargs).to(DEVICE)
...
def open_image(path: str, resolution: int):
    image = TF.to_tensor(Image.open(path)).to(DEVICE).unsqueeze(0)[:, :3]
