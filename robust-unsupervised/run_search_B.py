"""
run_search_B.py  —  LR Search Config B: Balanced LRs (default baseline)
========================================================================
NGD → Adam → LBFGS  with BALANCED adaptive LRs.

Hypothesis: moderate LRs closely match the original paper's default scaling,
providing a reference baseline between the high-LR (A) and low-LR (C) configs.

L  → Adam lr = 0.02 × 0.40 = 0.0080 | LBFGS lr = 0.005 × 0.30 = 0.00150
XL → Adam lr = 0.02 × 0.20 = 0.0040 | LBFGS lr = 0.005 × 0.10 = 0.00050

Usage (Kaggle Notebook 2):
    python run_search_B.py \
        --name search_B \
...
# ── Config B: BALANCED LRs — standard adaptive scaling ───────────────────────
ADAM_SCALE = {
    "XS": 1.00,
    "S":  0.80,
    "M":  0.60,
...
if __name__ == '__main__':
    ...
    for task in tasks:
        ...
        print(f"\n[B] {key}  lr_W={lr_W:.4f}  lr_W+={lr_Wp:.5f}  lr_W++={lr_Wpp:.5f}")
    ...
    print("\n\n=== SEARCH B (Balanced LRs) FINAL SCORES ===")
    print_table(scores)
