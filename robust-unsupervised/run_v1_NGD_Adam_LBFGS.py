"""
run_v1_NGD_Adam_LBFGS.py  —  Ablation Variant 1
================================================
Optimizer order:
    Phase 1  W    NGD    fixed LR  (gradient normalisation handles scale)
    Phase 2  W+   Adam   adaptive LR scaled by degradation severity
    Phase 3  W++  LBFGS  adaptive LR scaled by degradation severity

Rationale:
    NGD in Phase 1 gives a stable, scale-invariant first pass in W space.
    Adam in Phase 2 uses momentum to refine per-layer codes in W+.
    LBFGS in Phase 3 uses curvature info for a precise final fit in W++.

Usage (Kaggle):
    python run_v1_NGD_Adam_LBFGS.py --name v1_NGD_Adam_LBFGS \
...
def get_level_str(level) -> str:
    """Normalises task.level (str or int) to a string key for the LR tables."""
...
if __name__ == '__main__':
    ...
    for task in tasks:
        ...
        # Phase 1: NGD — fixed (gradient normalisation already scale-invariant)
        lr_W   = config.global_lr_scale * 0.08
        # Phase 2: Adam — scale down for severe degradations (noisy gradients)
        lr_Wp  = config.global_lr_scale * 0.02 * ADAM_SCALE[level_str]
        # Phase 3: LBFGS — scale down for severe degradations (rough landscape)
        lr_Wpp = config.global_lr_scale * 0.005 * LBFGS_SCALE[level_str]

        print(f"\n[V1] {task_key}  level_str={level_str}"
    ...
    print("\n\n=== V1 (NGD → Adam → LBFGS) FINAL SCORES ===")
