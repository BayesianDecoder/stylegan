"""
run_search_A.py  —  LR Search Config A: High LRs (accuracy-focused)
====================================================================
NGD → Adam → LBFGS  with HIGH adaptive LRs.

Hypothesis: higher LRs let Adam/LBFGS fit the degraded image more closely
→ better Accuracy LPIPS, may trade off some pFID realism.

L  → Adam lr = 0.02 × 0.60 = 0.0120 | LBFGS lr = 0.005 × 0.60 = 0.00300
XL → Adam lr = 0.02 × 0.40 = 0.0080 | LBFGS lr = 0.005 × 0.30 = 0.00150

Usage (Kaggle Notebook 1):
    python run_search_A.py \
        --name search_A \
...
# ── Config A: HIGH LRs — push Adam and LBFGS harder ─────────────────────────
ADAM_SCALE = {
    "XS": 1.00,
    "S":  0.90,
    "M":  0.80,
    "L":  0.60,   # <-- higher than baseline
    "XL": 0.40,   # <-- higher than baseline
}
LBFGS_SCALE = {
    "XS": 1.00,
    "S":  0.90,
...
if __name__ == '__main__':
    ...
    for task in tasks:
        ...
        print(f"\n[A] {key}  lr_W={lr_W:.4f}  lr_W+={lr_Wp:.5f}  lr_W++={lr_Wpp:.5f}")
    ...
    print("\n\n=== SEARCH A (High LRs) FINAL SCORES ===")
    print_table(scores)
