"""
run_v2_LBFGS_NGD_NGD.py  —  Ablation Variant 2
================================================
Optimizer order:
    Phase 1  W    LBFGS  adaptive LR scaled by degradation severity
    Phase 2  W+   NGD    fixed LR  (gradient normalisation handles scale)
    Phase 3  W++  NGD    fixed LR  (gradient normalisation handles scale)

Rationale:
    LBFGS in Phase 1 uses curvature info for a fast, high-quality initial W.
    NGD in Phases 2 & 3 gives stable, direction-normalised fine-tuning in
    the higher-dimensional W+ and W++ spaces without momentum accumulation.

Usage (Kaggle):
    python run_v2_LBFGS_NGD_NGD.py --name v2_LBFGS_NGD_NGD \
...
ADAM_SCALE = {          # kept here for completeness; not used in V2
    "XS": 1.00,
    "S":  0.80,
    "M":  0.60,
    "L":  0.40,
...
if __name__ == '__main__':
    ...
    for task in tasks:
        ...
        lr_W   = config.global_lr_scale * 0.08 * LBFGS_SCALE[level_str]
        # Phase 2: NGD — fixed (gradient normalisation already scale-invariant)
        lr_Wp  = config.global_lr_scale * 0.02
        # Phase 3: NGD — fixed
        lr_Wpp = config.global_lr_scale * 0.005

        print(f"\n[V2] {task_key}  level_str={level_str}"
